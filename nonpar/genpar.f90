...
!.... read the initial condition
      ...
      print*, 'Please, enter the format to use, 1=bl_sim.out ==> '
      read *, frmt

      if ( frmt .eq. 1) then  ! do the bl_sim.out format  
         do j=1,nin
            read(4,*) dum, dum, uin(j), dum, dum
         end do
         close(4)
      else         
         do j = 1, nin
            read(4,*,err=100,end=100) dum, uin(j), dum         ! my format
