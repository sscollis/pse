...
!.... local variables
      ...
      real :: c1(ny), c2(ny),  rdum, ar1, ai1, ar, ai
...
!.... Read in alpha from pse in my format
...
!.... initialize the field
	...
	ke(i) = ke(i) + opi(j)*(abs(ul(j))**2 + abs(vl(j))**2 + &
      ...
      if (norm.eq. 0 .or. norm .eq. 5 ) then
...
!.... Build the RHS for du/dalpha
	...
	do ldof = 1, ndof
	  do l = 1, ny
        ...
        Ab = two * iota * Ex(l,ldof,mdof) / (x(i+1)-x(i))
...
!.... save a field file
      ...
      open(71,file='dalpha.adj')
      write(71, "(I4,6E16.7)") (i, x(i), abs(amp(i,k,n)), &
                     real(amp(i,k,n)), aimag(amp(i,k,n)), & 
          real(alpha(i,k,n)), aimag(alpha(i,k,n)), i=is,ie)
      close(71)


!.... output some of my files

      if ( norm .eq. 5 ) then
         nxl =ie - is +1 
         open(10,file='afield.al',form='unformatted')
         write(10) nxl, ny
         write(10) (((ua(i,j,k),j=1,ny),i=is,ie),k=1,4)
         ...
         write(1) nxl, ny
         write(1) (((u(i,j,1,1,k),j=1,ny),i=is,ie),k=1,4)
         write(1) (x(i), i= is,ie)
         write(1) (y(j), j= 1,ny)
         close(1) 
         ...
         write (10) nxl, ny
         write (10) (((amp(i,1,1) * u(i,j,1,1,k), j= 1,ny), i= is,ie), k= 1,4)
         write (10) (x(i), i= is,ie)
         write (10) (y(j), j= 1,ny)
         close (10)
