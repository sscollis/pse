...
!     8-12-99:  1) Added Newton update for alpha
!==============================================================================
      use global
      use int_str
      use fmax
...
!	  if (i.ne.is) alpha(i,1,1) = -cmplx(alphar, alphai)
...
!.... only consider only one mode for linear analysis
      ...
      if (ipfix.eq.2) alpha(:,k,n) = alphah(:)
...
!.... initialize the field
	...
	do j = 1, ny
	  jprod(i) = jprod(i) + opi(j)*( &
                     (ut(i,j,1)*ul(j)+ut(i,j,2)*vl(j)) * ub(j,i) + &
                      ut(i,j,1)*pl(j) + ut(i,j,4)*ul(j) - &
                      rei*iota*(alpha(i,j,n)-alphat(i)) * &
...
!.... Build the RHS for du/dalpha
	...
	r = czero
        ...
        Ab = iota * A + two * alphal * Ex - three * iota * Ex / (x(i)-x(i-1))
...
!         dfda = dfda / (x(i)-x(i-1))
          ...
          do j = 1, ny
            Jprod(i) = Jprod(i) + opi(j)*( &
		       ...
		       ut(i,j,1)*pl(j) + ut(i,j,4)*ul(j) - &
                       ...
                       (ut(i,j,1)*ul(j)+ut(i,j,2)*vl(j)))
            dfda = dfda + opi(j)*( &
                       (ut(i,j,1)*ua(i,j,1)+ut(i,j,2)*ua(i,j,2)) * ub(j,i) + &
	  ...
	  Jprodx = (Jprod(i)-Jprod(i-1))/(x(i)-x(i-1))
          dfda = one
          f = zero ! Jprodx
          if (.not. newton) dfda = zero ! -iota * Jprod(i)	  
	end if
