...
!.... Build the RHS for du/dalpha
	...
	r = czero
        ...
        Ab = iota * A + two * alphal * Ex - two * iota * Ex / (x(i)-x(i-1))
...
!	    if (abs(kex/ke(i))/abs(alpha(i,k,n)) .lt. tol) exit
            ...
            write(*,"('->',i2,1x,6(1pe13.6,1x))") iter, alpha(i,k,n), &
                 abs(amp(i,k,n)), ke(i), abs(kex/dfda)/abs(alpha(i,k,n))
            if (abs(kex/dfda) .lt. tol) exit
          endif
	end if
