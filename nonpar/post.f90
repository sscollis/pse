...
!.... write only the hat field file, here I assumed that mt=1 and nz=1
      if (isign .eq. 1) then 
         open(1,file='field.hat',form='unformatted')
         write(1) nxl, ny
         write(1) (((u(i,j,1,1,idof),j=1,ny),i=is,ie),idof=1,4)
         write(1) (x(i), i= is,ie)
         write(1) (y(j), j= 1,ny)
         close(1)
      end if
