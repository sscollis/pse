...
!.... Read in alpha from a regular PSE run (assumes same mesh)
      ...
      if (norm.eq.3.or.norm.eq.4.or.norm.eq.5) then
...
!.... read APSE field file
        ...
        i = is
        do j = 1, ny
          Jprod(i) = Jprod(i) + opi(j)*( &
                     (uh(i,j,1)*ut(i,j,1)+uh(i,j,2)*ut(i,j,2)) * ub(j,i) + &
                     uh(i,j,1)*ut(i,j,4) + uh(i,j,4)*ut(i,j,1) - &
        ...
        do i = is+1, ie
          ...
          do j = 1, ny
            Jprod(i) = Jprod(i) + opi(j)*( &
                       ...
                       rei*iota*(alphah(i)+alphat(i)) * &
                       (uh(i,j,1)*ut(i,j,1)+uh(i,j,2)*ut(i,j,2)))
            dfdat(i) = dfdat(i) + opi(j)*( &
                       (uh(i,j,1)*uta(i,j,1)+uh(i,j,2)*uta(i,j,2)) * ub(j,i) +&
                       uh(i,j,1)*uta(i,j,4) + uh(i,j,4)*uta(i,j,1) - &
                       rei*iota*(uh(i,j,1)*ut(i,j,1)+uh(i,j,2)*ut(i,j,2)) - &
                       rei*iota*(alphah(i)+alphat(i)) * &
                       (uh(i,j,1)*uta(i,j,1)+uh(i,j,2)*uta(i,j,2)))
            dfdah(i) = dfdah(i) + opi(j)*( &
                       (ut(i,j,1)*uha(i,j,1)+ut(i,j,2)*uha(i,j,2)) * ub(j,i) +&
                       ut(i,j,1)*uha(i,j,4) + ut(i,j,4)*uha(i,j,1) - &
                       rei*iota*(ut(i,j,1)*uh(i,j,1)+ut(i,j,2)*uh(i,j,2)) - &
                       rei*iota*(alphah(i)+alphat(i)) * &
                       (ut(i,j,1)*uha(i,j,1)+ut(i,j,2)*uha(i,j,2)))
          end do
          f1 = Jprod(i) - Jprod(i-1)
          f2 = alphah(i) - alphat(i)
          det = -dfdah(i) - dfdat(i)
          dalphah = -(-one * f1 - dfdat(i) * f2) / det
          dalphat = -(-one * f1 + dfdah(i) * f2) / det
          alphah(i) = alphah(i) + dalphah
          alphat(i) = alphat(i) + dalphat
        end do
        do i = is, ie
          write(22,"(6(1pe13.6,1x))") x(i), &
                                      real(alphah(i)), aimag(alphah(i)), &
                                      real(alphat(i)), aimag(alphat(i)), &
                                      abs(Jprod(i))
        end do
        call exit(0)
        end if
...
!.... initialize the field
      ...
      if (norm.eq.5) then
        ...
        do j = 1, ny
          Jprod(i) = Jprod(i) + opi(j) * ( &
            (uh(i-1,j,1)*u(i,j,k,n,1)+uh(i-1,j,2)*u(i,j,k,n,2))*ub(j,i) + &
...
!.... initialize the field
      ...
      if (norm.eq.0) then
        ...
        do j = 1, ny
          jprod(i) = jprod(i) + opi(j)*( &
                     (uh(i,j,1)*ul(j)+uh(i,j,2)*vl(j)) * ub(j,i) + &
...
!.... compute the mode amplitude
        ...
        if (norm.eq.0) then
          ...
          do j = 1, ny
            Jprod(i) = Jprod(i) + opi(j) * ( &
              (uh(i,j,1)*u(i+1,j,k,n,1)+uh(i,j,2)*u(i+1,j,k,n,2))*ub(j,i+1) + &
...
!.... Hack for discrete adjoint (need matrix at i+1)
        ...
        Ap1(:,1,1) = c1(:) * ub(:,i+1)
        Ap1(:,1,2) = -rei * two * c1(:) * c2(:)
        Ap1(:,1,4) = c1(:)
        Ap1(:,2,1) = rei * two * c1(:) * c2(:)
        Ap1(:,2,2) = c1(:) * ub(:,i+1)
        Ap1(:,3,3) = c1(:) * ub(:,i+1)
        Ap1(:,4,1) = c1(:)
...
!.... fix the streamwise pressure gradient
        ...
        Ab = Ap1
        if ( (ipfix.eq.1) .or. &
             (ipfix.eq.2 .and. i.eq.ie-1) ) then
          Ab(:,:,4) = zero
        end if

        Ab = Ab - two * iota * alpha(i+1,k,n) * Exp1
        ...
        if (norm.eq.0) then       ! following Herbert AIAA-93-3053
          ...
          do j = 1, ny
            Jprod(i) = Jprod(i) + opi(j)*( &
                       ...
                       (uh(i,j,1)*ul(j)+uh(i,j,2)*vl(j)))
            dfda = dfda + opi(j)*( &
                       (uh(i,j,1)*ua(i,j,1)+uh(i,j,2)*ua(i,j,2)) * ub(j,i) + &
          ...
          do j = 1, ny
            ...
            dfdat(i) = dfdat(i) + opi(j)*( &
                       ...
                       (uh(i,j,1)*ua(i,j,1)+uh(i,j,2)*ua(i,j,2)))
            dfdah(i) = dfdah(i) + opi(j)*( &
                       (ul(j)*uha(i,j,1)+vl(j)*uha(i,j,2)) * ub(j,i) + &
                       ul(j)*uha(i,j,4) + pl(j)*uha(i,j,1) - &
                       rei*iota*(ul(j)*uh(i,j,1)+vl(j)*uh(i,j,2)) - &
                       rei*iota*(alphah(i)+alpha(i,k,n)) * &
                       (ul(j)*uha(i,j,1)+vl(j)*uha(i,j,2)))
          end do
          f1 = Jprod(i) - Jprod(i+1)
          f2 = alphah(i) - alpha(i,k,n)
          det = -dfdah(i) - dfdat(i)
          dalphah = -(-one * f1 - dfdat(i) * f2) / det
          dalphat = -(-one * f1 + dfdah(i) * f2) / det
          alphah(i)    = alphah(i)    + dalphah
          alpha(i,k,n) = alpha(i,k,n) + dalphat
        else if (norm.eq.5) then
          Jprod(i) = czero
          do j = 1, ny
            Jprod(i) = Jprod(i) + opi(j) * ( &
...
!.... convergence check
        ...
        if (norm.eq.0) then
          ...
          write(*,"('->',i2,1x,6(1pe13.6,1x))") iter, alpha(i,k,n), &
                abs(amp(i,k,n)), ke(i), abs(f/dfda)/abs(alpha(i,k,n))
        else if (norm.eq.3.or.norm.eq.5) then
          write(*,"('->',i2,1x,6(1pe13.6,1x))") iter, alpha(i,k,n), &
                abs(amp(i,k,n)), abs(Jprod(i)), abs(f/dfda)/abs(alpha(i,k,n))
        else if (norm.eq.4) then
          write(*,"('->',i2,1x,7(1pe13.6,1x))") iter, alpha(i,k,n), &
                abs(amp(i,k,n)), abs(Jprod(i)), &
                abs(dalphat)/abs(alpha(i,k,n)), abs(dalphah)/abs(alphah(i))
        end if

        if (abs(f/dfda)/abs(alpha(i,k,n)).lt.tol .and. norm.ne.4) exit
...
!.... save a non interpolated field file
      ...
      open  (10, file='field.pse', form='unformatted')
      write (10) ie-is+1, ny
      write (10) (((uh(i,j,idof), j= 1,ny), i= is,ie), idof= 1,4)
      write (10) (x(i), i= is,ie)
      write (10) (y(j), j= 1,ny)
      write (10) (amph(i), i = is, ie)
      write (10) (((uha(i,j,idof), j= 1,ny), i= is,ie), idof= 1,4)
      write (10) (alphah(i), i= is,ie)
      close (10)
      end if
...
!           uh(i,j,4)*u(i+1,j,k,n,1) + &
            ...
            (uh(i,j,1)*u(i+1,j,k,n,1)+uh(i,j,2)*u(i+1,j,k,n,2)) )
          Jprod3 = Jprod3 + opi(j) * ( &
            (uh(i,j,1)*u(i,j,k,n,1)+uh(i,j,2)*u(i,j,k,n,2))*ub(j,i) + &
...
!.... compute growth rate based on various other quantitities
      ...
      call growth(ie-(is+1)+1, ny, x(is+1:ie), y, amp(is+1:ie,k,n), &
