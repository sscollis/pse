...
!==============================================================================
      ...
      complex, allocatable :: u1(:,:,:), amp1(:), alpha1(:)
...
!.... Read in alpha from a regular PSE run (assumes same mesh)
      ...
      if (norm.ne.3) pse_alpha = .false.
      if (pse_alpha) then
        allocate( alpha1(nx), u1(nx,ny,ndof), amp1(nx) )
        open(20,err=100,file='pse.in')
        do i = is, ie
          read(20,*) rdum, alphar, alphai
!         alpha(i,1,1) = cmplx(alphar, alphai)
          alpha1(i) = cmplx(alphar, alphai)
        end do
        close(20)
...
!.... read PSE field file
        ...
        read (10) (((u1(i,j,idof), j= 1,ny), i= is,ie), idof= 1,4)
        read (10) (rdum, i= is,ie)
        read (10) (rdum, j= 1,ny)
        read (10) (amp1(i), i = is, ie)
        close (10)
      ...
      if (norm.eq.0) then
        ...
        do j = 1, ny
          jprod(i) = jprod(i) + opi(j)*( &
                     (u1(i,j,1)*ul(j)+u1(i,j,2)*vl(j)) * ub(j,i) + &
                      u1(i,j,1)*pl(j) + u1(i,j,4)*ul(j) - &
                      rei*iota*(alpha1(i)-alpha(i,k,n)) * &
                      (u1(i,j,1)*ul(j)+u1(i,j,2)*vl(j)))
        end do
      else 
...
!         write(88,"(8(1pe13.6,1x))") x(i), real(jprod(i)), aimag(jprod(i)), &
!                                     abs(jprod(i))
        end if
...
!       write(*,"('WARNING:  The curvature terms may be wrong')")
...
!.... compute the correction for alpha (following Herbert AIAA-93-3053)
        ...
        if (norm.eq.0) then       ! following Herbert AIAA-93-3053
          ...
          alpha(i,k,n) = (one-sor)*alpha(i,k,n) + &
                              sor*(alpha(i,k,n) - iota * Jprodx / Jprod(i))
        end if
        end if
...
!         if (abs(kex/ke(i))/abs(alpha(i,k,n)) .lt. tol) exit
          ...
          write(*,"('->',i2,1x,6(1pe13.6,1x))") iter, alpha(i,k,n), &
                abs(amp(i,k,n)), abs(Jprod(i)), abs(Jprodx/Jprod(i))
!         if (abs(Jprodx/Jprod(i))/abs(alpha(i,k,n)) .lt. tol) exit
          if (abs(Jprodx/Jprod(i)) .lt. tol) exit
        end if
