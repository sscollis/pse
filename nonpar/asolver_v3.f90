...
!==============================================================================
      ...
      complex :: jprod(nx), jprodx, uah(nx,ny,ndof), alphad(nx)
...
!.... Read in alpha from a regular PSE run (assumes same mesh)
	...
	read (10) (((rdum, j= 1,ny), i= is,ie), idof= 1,4)
	read (10) (alphah(i), i= is, ie)
        close (10)
      end if

      if (ipfix .eq. 2 ) then
         open (10, file='afield.adj', form='unformatted')
         read (10) nxi, nyi
         if (nxi.ne.(ie-is+1) .or. nyi.ne.ny) &
              call error('asolver$','Illegal field.pse file$')
         read (10) (((uah(i,j,idof), j= 1,ny), i= is,ie), idof= 1,4)
         read (10) (rdum, i= is,ie)
         read (10) (rdum, j= 1,ny)
         read (10) (amph(i), i = is, ie)
         read (10) (((rdum,  j= 1,ny), i= is,ie), idof= 1,4)
         read (10) (alphad(i), i= is, ie)
         close (10)
      end if
...
!.... only consider only one mode for linear analysis
	...
	ubci(:,k,n) = u(i,:,k,n,1) / Jprod(i)
	vbci(:,k,n) = u(i,:,k,n,2) / Jprod(i)
	wbci(:,k,n) = u(i,:,k,n,3) / Jprod(i)
	pbci(:,k,n) = u(i,:,k,n,4) / Jprod(i)
...
!.... add the pressure gradient to the right hand side
...
!.... compute growth rate based on various other quantitities
      ...
      i = is
      do j = 1, ny
        write(45,"(15(1pe13.6,1x))") y(j), opi(j), &
             uh(i,j,1), uh(i,j,2), uh(i,j,4), &
             amp(i,k,n)*u(i,j,k,n,1), amp(i,k,n)*u(i,j,k,n,2), &
             amp(i,k,n)*u(i,j,k,n,4), ub(j,i)
      end do
