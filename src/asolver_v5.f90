...
!.... Build the LHS matrix
        ...
        !.... Note that the stabilization constant is still positive! [SSC]

        fact = ( one + tau / (x(i+1)-x(i)) )
          ...
          do l = 1, ny
            ...
            do mdof = 1, ndof
              r(l0+ldof) = r(l0+ldof) + &
                 ...
                 Ab(l,ldof,mdof) * u(i+1,l,k,n,mdof) / (x(i+1)-x(i))
            end do
          end do
        end do
...
!.... Build the RHS for du/dalpha
        ...
        Ab = fact * ( iota * A + iota * Ex / (x(i+1)-x(i)) + &
                      two * alphal * Ex ) + &
        ...
        Ab = ( fact-one ) * &
             ( iota*A + two*alphal*Ex + iota*Ex/(x(i+1)-x(i)) ) + &
               two*iota*Ex/(x(i+1)-x(i))
