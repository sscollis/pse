...
!.... initialize
      ...
      do j = 1, nint
        yl(j) = ddot( ny, yint(j,1), nint, y, 1 )
      end do
...
!.... write out the ymax
      ...
      open (2, file="ymax.dat")
      do i= 1,nx
        write (2,"(11(1pe20.13,1x))") x(i), ylmax(i,:)
      end do
      close (2)
...
!==============================================================================
      ...
      real    :: dxb, dxf, ds, fc1, cc1, bc1, rei

      real    :: opy(ny,ny)
      complex :: uy(ny), vy(ny), wy(ny), lambda
      real    :: uby(ny,nx), vby(ny,nx), wby(ny,nx)
...
!.... output the wall shear stress components

      open (2, file="swall.dat")
      do i = 1, nx
        uy = rei * matmul( opy, u(i,:,1) )*amp(i)
        vy = rei * matmul( opy, u(i,:,2) )*amp(i)
        wy = rei * matmul( opy, u(i,:,3) )*amp(i)
        write(2,"(11(1pe20.13,1x))") x(i), &
             uy(1), vy(1), wy(1), abs(uy(1)), abs(vy(1)), abs(wy(1))
      end do
      close(2)

!.... output lambda

      open(2, file="lambda.dat")
      do i = 1, nx
        uy = rei * matmul( opy, u(i,:,1) ) * amp(i)
        vy = rei * matmul( opy, u(i,:,2) ) * amp(i)
        wy = rei * matmul( opy, u(i,:,3) ) * amp(i)
        lambda = -uy(1) * uby(1,i) - wy(1) * wby(1,i)
        write(2,"(11(1pe20.13,1x))") x(i), lambda, abs(lambda), &
                                     uby(1,i), wby(1,i)
      end do
