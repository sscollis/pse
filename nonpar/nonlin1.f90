...
!.... compute the total energy norm used to measure the nonlinear amplitude
	...
	do k = 1, nz
	  ...
	  wl = u(i,:,k,n,3)
	  do j = 1, ny-1
	    ke = ke + pt5 * ( conjg(ul(j))*ul(j) + &
                 conjg(vl(j))*vl(j) + conjg(wl(j))*wl(j) + &
...
!.... form the nonlinear terms
	...
	do n = 1, mt
	  do k = 1, nz
	    ...
	    ui(n,k, 4) = ( amp(i,k,n) * u(i,j,k,n,1) - &
                           amp(i-1,k,n) * u(i-1,j,k,n,1) ) / (x(i) - x(i-1))
	    ui(n,k, 5) = ( amp(i,k,n) * u(i,j,k,n,2) - &
                           amp(i-1,k,n) * u(i-1,j,k,n,2) ) / (x(i) - x(i-1))
	    ui(n,k, 6) = ( amp(i,k,n) * u(i,j,k,n,3) - &
                           amp(i-1,k,n) * u(i-1,j,k,n,3) ) / (x(i) - x(i-1))
