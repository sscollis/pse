!==============================================================================
      subroutine growth( nx, ny, x, y, amp, alpha, u, opi, nint, yint, opy, &
                         itype, ystr, re )
!==============================================================================
      use fmax
      ...
      integer :: i, j, ii, idof, jmax, itype
      real    :: f(ny), fr(ny), fi(ny), frmax, fimax, ymax
      real    :: q(9,nx), v(nint,ndof), vr(nint,ndof), vi(nint,ndof)
      real    :: utempr(ny), utempi(ny), uytempr(ny), uytempi(ny), dudy(nx)
      complex :: umax(nx,ndof)

      real    :: dxb, dxf, ds, fc1, cc1, bc1, maxy(ndof,nx), pin, re
      real    :: opy(ny,ny), wv2(nint), yc, ystr, yin(nint)
...
!.... create fine mesh in y
      ...
      do j = 1, nint
        wv2(j) =  cos((j-1) * pin)
        yc = .5 * (1. - wv2(j))
        yin(j) = ymax * ystr * yc / (1. + ystr - yc)
      end do
      
      open(5,file='yint.dat')
      open(6,file='y.dat')
      
      do j=1,nint
        write(5,'(2e16.7)') real(j), yin(j) 
      end do
      
      do j=1,ny
        write(6,'(2e16.7)') real(j), y(j) 
      end do
      close(5)
      close(6)
...
!.... write out the results
      ...
      do i= 1,nx
	write (2,"(11(1pe20.13,1x))") x(i), (q(idof,i), idof= 1,9), sqrt(re * x(i)) 
      end do
      close (2)
...
!.... write out the wall pressure
      ...
      do i= 1,nx
        write (2,"(11(1pe20.13,1x))") x(i), u(i,1,4)*amp(i), &
                                      abs(u(i,1,4)*amp(i)), sqrt(re * x(i))
      end do
      close (2)

!.... Compute some y derivatives at the wall ( j=1 )

      do i= 2,nx-1     
        do j= 1,ny
          utempr(j) = real (u(i,j,1))
          utempi(j) = aimag(u(i,j,1))
        end do
#ifdef CRAY
	call sgemv(trans, ny, ny, one, opy, ny, utempr, 1, &
	  zero, uytempr, 1)
	call sgemv(trans, ny, ny, one, opy, ny, utempi, 1, &
	  zero, uytempi, 1)
#else
	call dgemv(trans, ny, ny, one, opy, ny, utempr, 1, &
	  zero, uytempr, 1)
	call dgemv(trans, ny, ny, one, opy, ny, utempi, 1, &
	  zero, uytempi, 1)
#endif  
        dudy(i) = abs(cmplx(uytempr(1), uytempi(1))*amp(i))

!.... linearly interpolate the ends, later

      end do
         
!.... write out files in my format 


      if ( itype .eq. 0 ) then
        open ( 3, file='all.pse' )
        do i=1,nx
          write(3,'(11(1pe20.13,1x))') x(i), sqrt(re * x(i)), & 
               maxy(1,i),  q(1,i), dudy(i), abs(u(i,1,4)*amp(i))
        end do
        close(3)
      else if ( itype .eq. 2 ) then 
        open ( 4, file='all.adj' )
        do i=1,nx
          write(4,'(11(1pe20.13,1x))') x(i), sqrt(re * x(i)), & 
               maxy(1,i), q(1,i), dudy(i), abs(u(i,1,4)*amp(i))
        end do
        close(4)
      end if
